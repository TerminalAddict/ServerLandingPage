...
REMOTE_PATH := /var/www/html/ServerLandingPage
REMOTE_USER := paul
URL := https://hosting.netenterprises.co.nz/ServerLandingPage
BLOGNAME := ServerLandingPage
...
build: include-npm-deps
	@cd assets/js; closure-compile custom.js
	@cat $(VENDOR_DIR)/jquery.min.js <(echo) $(VENDOR_DIR)/popper.min.js <(echo) $(VENDOR_DIR)/bootstrap.min.js <(echo) assets/js/custom.min.js  > $(JSBUNDLE_DIR)/bundle.js
...
PingSearchEngine:
	@echo "NOT Pinging ping-o-matic, Google, and Bing"
